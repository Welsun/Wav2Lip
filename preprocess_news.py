# 对数据集视频做了预处理，提取每一个视频片段的视频帧和相对应的语音，只截取人脸部分的图片
...
def process_video_file(vfile, args):
    vidname = os.path.basename(vfile).split('.')[0] #视频名字
    #dirname = vfile.split('/')[-2]#上级文件夹名字 video_segments
...
def main(args):
    ...
    filelist = sorted(glob(args.data_root + "/*.mp4"))  # 获取所有的mp4文件名
    for f in tqdm(filelist):
        process_video_file(f, args)
        #process_audio_file(f, args)
